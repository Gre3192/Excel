Attribute VB_Name = "Calcola_SLU"
'namespace=vba-files\Moduli

Sub calcola_combinazione_SLU()
    ...
    '-- CARICO G1 ----------------------------------------------------------------------------------------
        ...
        Else
            ...
            For i = 1 To tot
                ...
                condition = ws.Cells(next_row, start_col + 6).Value
                analysis = ws.Cells(next_row, start_col + 8).Value
                state = ws.Cells(next_row, start_col + 9).Value
                g1 = IIf(IsEmpty(Qinput) Or Not IsNumeric(Qinput) Or state <> "Attivo", 0, Qinput)
                tot_qsl_g1_NTC08 = tot_qsl_g1_NTC08 + g1 * getGamma("NTC08", "SLU", "G1", condition, analysis)
                tot_qsl_g1_NTC18 = tot_qsl_g1_NTC18 + g1 * getGamma("NTC18", "SLU", "G1", condition, analysis)
                ' Debug.Print "(NTC08) (" & i & ") Qslu_G1 = G1 * gamma = " & g1 & " * " & getGamma("NTC08","SLU", "G1", condition, analysis) & " = " & g1 * getGamma("NTC08","SLU", "G1", condition, analysis)
                ' Debug.Print "(NTC18) (" & i & ") Qslu_G1 = G1 * gamma = " & g1 & " * " & getGamma("NTC18","SLU", "G1", condition, analysis) & " = " & g1 * getGamma("NTC18","SLU", "G1", condition, analysis)
            Next
        End If
        ' Debug.Print "(NTC08) SOMMA Qslu_G1 = " & tot_qsl_g1_NTC08 & vbCrLf & "--------------------------------------------"
        ' Debug.Print "(NTC18) SOMMA Qslu_G1 = " & tot_qsl_g1_NTC18 & vbCrLf & "--------------------------------------------"
    '
    '-- CARICO G2 ----------------------------------------------------------------------------------------
        ...
        Else
            ...
            For i = 1 To tot
                ...
                condition = ws.Cells(next_row, start_col + 6)
                analysis = ws.Cells(next_row, start_col + 8)
                state = ws.Cells(next_row, start_col + 9).Value
                g2 = IIf(IsEmpty(Qinput) Or Not IsNumeric(Qinput) Or state <> "Attivo", 0, Qinput)
                tot_qsl_g2_NTC08 = tot_qsl_g2_NTC08 + g2 * getGamma("NTC08", "SLU", "G2", condition, analysis)
                tot_qsl_g2_NTC18 = tot_qsl_g2_NTC18 + g2 * getGamma("NTC18", "SLU", "G2", condition, analysis)
                ' Debug.Print "(NTC08) (" & i & ") Qslu_G2 = G2 * gamma = " & g2 & " * " & getGamma("NTC08","SLU", "G2", condition, analysis) & " = " & g2 * getGamma("NTC08","SLU", "G2", condition, analysis)
                ' Debug.Print "(NTC18) (" & i & ") Qslu_G2 = G2 * gamma = " & g2 & " * " & getGamma("NTC18","SLU", "G2", condition, analysis) & " = " & g2 * getGamma("NTC18","SLU", "G2", condition, analysis)
            Next
        End If
    ...
    '-- CARICO Qk ----------------------------------------------------------------------------------------
        ...
        Else
            ...
            qkPrinc_Num = getQkPrincArray("NTC18", "NotNum", "SLU", "Qk", tot, rangeNum, rangeCorr, rangeQinput, rangeCondition, rangeAnalysis, rangeCategory, rangeState, "getQkPrincNum")
            qkPrinc_Category = getQkPrincArray("NTC18", "NotNum", "SLU", "Qk", tot, rangeNum, rangeCorr, rangeQinput, rangeCondition, rangeAnalysis, rangeCategory, rangeState, "getQkPrincCategory")
            qsl_qkPrinc_NTC08 = getQkPrincArray("NTC08", "NotNum", "SLU", "Qk", tot, rangeNum, rangeCorr, rangeQinput, rangeCondition, rangeAnalysis, rangeCategory, rangeState)
            qsl_qkPrinc_NTC18 = getQkPrincArray("NTC18", "NotNum", "SLU", "Qk", tot, rangeNum, rangeCorr, rangeQinput, rangeCondition, rangeAnalysis, rangeCategory, rangeState)
            
            qsl_qkSecon_NTC08 = getQkSeconArray("NTC08", 0, "SLU", "Qk", tot, rangeCorr, rangeQinput, rangeCondition, rangeAnalysis, rangeCategory, rangeState)
            qsl_qkSecon_NTC18 = getQkSeconArray("NTC18", 0, "SLU", "Qk", tot, rangeCorr, rangeQinput, rangeCondition, rangeAnalysis, rangeCategory, rangeState)
    ...
    '
    '-- COMBINAZIONE SLU ---------------------------------------------------------------------------------
        start_row = ws.Range(range_pointer("SLU")).Row
        start_col = ws.Range(range_pointer("SLU")).Column
